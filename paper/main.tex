...
\emph{Context:} Graphs and various graph-like combinatorial structures, such as
...
\emph{Inquiry:} The previous formulation of algebraic graphs did not support
edge labels, which was a serious practical limitation. This paper redesigns the
main algebraic data type and removes this limitation.

\emph{Approach:} We follow a fairly standard approach of parameterising a data
structure with a semiring of edge labels. The newly developed data structure is
both more general and also simpler: the two ways of composing graphs used in the
original formulation can now be obtained from a single operation by fixing the
semiring parameter to zero and one, respectively.
...
% We need parallel composition too!
...
% Naturally ordered
% Use trees/united monoids for modelling parallel/sequential composition
% What about tries?
% Equivalence classes?
% Languages
